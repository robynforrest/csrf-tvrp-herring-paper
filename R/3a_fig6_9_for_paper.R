...
fig7 <- list() #SSB
fig8 <- list() #SB0
fig9a <- list() #LRP increasing M
fig9b <- list() #PLRP increasing M
...
for(j in 1:nstocks){
  ...
  g4supp <- LRP |>
    filter(year>cyr) |>
    mutate(group=factor(scenario, levels=scenameHuman)) |>
    filter(scenario=="Random Walk in M") |>
    ggplot()+
    geom_ribbon(aes(x=year, ymin=lwr , ymax=upr, fill=`SB0 type`), alpha = 0.1)+
    geom_line(aes(x=year, y=med, col=`SB0 type`, lty=`SB0 type`),lwd=1.25)+
  ...
  g4 <- LRP |>
    filter(year>cyr) |>
    mutate(group=factor(scenario, levels=scenameHuman)) |>
    filter(scenario=="Random Walk in M Drift") |>
    ggplot()+
    geom_ribbon(aes(x=year, ymin=lwr , ymax=upr, fill=`SB0 type`), alpha = 0.1)+
    geom_line(aes(x=year, y=med, col=`SB0 type`, lty=`SB0 type`),lwd=1.25)+
    geom_vline(xintercept=cyr, lty=2)+
  ...
  ggsave(file.path(StockDirFigs, paste0("MSE-allLRP_proYear_NF_increaseM.png")),
         width = 16, height = 10)
  ...
  PLRPB0_NF_constM <- PLRPB0_NF |> filter(scenario=="Random Walk in M")
  PLRPB0_NF_incrM <-  PLRPB0_NF |> filter(scenario=="Random Walk in M Drift")

  g5 <- plotPLRP_SB0(PLRPB0_NF_incrM,
                scentext=TRUE,
                panel=FALSE)
  g5

  ggsave(file.path(StockDirFigs, paste0("MSE-PLRP_B0_allScen_NF_increasingM.png")),
         width = 16, height = 10)

  g5supp <- plotPLRP_SB0(PLRPB0_NF_constM,
                     scentext=TRUE,
                     panel=FALSE)
  g5supp

  ggsave(file.path(StockDirFigs, paste0("MSE-PLRP_B0_allScen_NF_constantM.png")),
         width = 16, height = 10)
